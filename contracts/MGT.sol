...
contract MGT is Initializable, ERC20Upgradeable, PausableUpgradeable, OwnableUpgradeable {
    ...
    function initialize() initializer public {
        __ERC20_init("MetaGamificationToken", "MGT");
        __Pausable_init();
        __Ownable_init();
    ...
    function withdraw(address recipient, uint256 amount) external virtual {
        require(_msgSender() != address(0) && _msgSender() == _rewardAddress, "MGT: Invalid sender");
    ...
    function transferFrom(
        ...
        require(!_isLockTransferForAntiBot || sender == _lpCreatorAddress, "MGT: Antibot revert");
        if (isAddressLP(recipient)) {
            // SELL
            uint256 _fee = 0;
            if (sender != _lpCreatorAddress) {
                require(_limiter.isValidSell(amount), "MGT: Sell limit reach");
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        if (isAddressLP(_msgSender()) || isAddressLP(recipient)) {
            // BUY
            require(!_isLockTransferForAntiBot || recipient == _lpCreatorAddress, "MGT: Antibot revert");
            uint256 _fee = 0;
            if (recipient != _lpCreatorAddress && recipient != _pancakeRouterAddress) {
                require(_limiter.isValidBuy(recipient, amount), "MGT: Buy limit reach");
            ...
            require(recipient != address(this), "MGT: Failed to transfer");
